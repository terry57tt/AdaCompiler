...
public class SemanticTest {
    ...
    @Test
    public void testTypeAccess() throws IOException {
        SyntaxTree ast = createAST("example/SemanticDemo/demo_1.ada");
        SemanticControls.setName_file(FileHandler.getFileName("demo_1.ada"));
        Semantic semantic = new Semantic(ast);
        List<String> errors = SemanticControls.getErrors();
        assert errors.isEmpty() || errors.stream().noneMatch(s -> s.contains("controleSemantiqueTypeAccess"));
        errors.clear();
    }

    @Test
    public void testTypeRecord() throws IOException {
        SyntaxTree ast = createAST("example/SemanticDemo/demo_1.ada");
        SemanticControls.setName_file(FileHandler.getFileName("demo_1.ada"));
        Semantic semantic = new Semantic(ast);
        List<String> errors = SemanticControls.getErrors();
        assert errors.isEmpty() || errors.stream().noneMatch(s -> s.contains("controleSemantiqueTypeRecord"));
        errors.clear();
    }
}
