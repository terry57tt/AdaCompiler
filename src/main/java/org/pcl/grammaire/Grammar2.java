...
public class Grammar2 {
    ...
    public Grammar2(ArrayList<Token> tokens, String file){
        this.tokens = tokens;
        this.file = file;
    }

    public Grammar2(ArrayList<Token> tokens){
        this(tokens, "test");
    }
...
//            GrammarErrorUtility.ProceedAnalysis(expectedMessage, this, currentToken.getLineNumber(), file);
    ...
    public static  Grammar2 createGrammarError(Grammar2 grammar, int decal, Token newToken, String file) {
        Grammar2 grammarError = new Grammar2(GrammarErrorUtility.deepClone(grammar.tokens), file);
    ...
    public void meaningfulNode() {
        ...
        while (!nodes_to_visit.isEmpty()) {
            ...
            if (!currentNode.isFinal()) {
                ...
                // affectation
                else if (currentNode.getValue().equals("nodeIntr2prime") && currentNode.getParent().getValue().equals("nodeIntr1") && currentNode.firstChild().getValue().equals(":=")) {
                    currentNode.getChildren().remove(0); //delete node :=
                ...
                //end
                if (currentNode.getValue().equals("end") && currentNode.getToken().getType()==TokenType.KEYWORD) {
                    currentNode.deleteFromParent();
                }
                //end if
                if(currentNode.getValue().equals("if") && currentNode.getToken().getType()==TokenType.KEYWORD
